...
??6SimpleLogger@Logging@TechnicalServices@@UEAAAEAV012@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z$rtcName$0 DB 06eH ; TechnicalServices::Logging::SimpleLogger::operator<<
	...
	DB	00H
	ORG $+4
??6SimpleLogger@Logging@TechnicalServices@@UEAAAEAV012@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z$rtcName$1 DB 064H ; TechnicalServices::Logging::SimpleLogger::operator<<
	DB	065H
	DB	073H
	DB	074H
	DB	069H
	...
	DB	00H
	ORG $+12
??6SimpleLogger@Logging@TechnicalServices@@UEAAAEAV012@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z$rtcVarDesc DD 048H ; TechnicalServices::Logging::SimpleLogger::operator<<
	DD	024H
	DQ	FLAT:??6SimpleLogger@Logging@TechnicalServices@@UEAAAEAV012@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z$rtcName$1
	DD	028H
	DD	08H
	...
	ORG $+96
??6SimpleLogger@Logging@TechnicalServices@@UEAAAEAV012@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z$rtcFrameData DD 02H ; TechnicalServices::Logging::SimpleLogger::operator<<
	DD	00H
...
; 55   :     *this << "Simple Logger shutdown successfully";
...
; 63   :     
; 64   :     // Updated 2020-Sep-03
; 65   :     //
; 66   :     // C++20 provides a much needed refresh to the Chrono library, but until then I have to used some C functions to get the time
; 67   :     // formatted how I like.  And even then, MS Visual has (correctly imo) deprecated some of the C functions because they are
; 68   :     // inherently unsafe - in this case the std::localtime() function.  So I'm going to limp along here until C++20 is released -
; 69   :     // but (note to self) revisit this and rewrite when the time comes.
; 70   :     #if defined(_MSC_VER)  &&  _MSC_VER >= 1920     // if using Visual Studio 2019 version 16.0 or earlier  (I believe VSC++ started enforcing the deprecation here)
; 71   :       std::tm destination;
; 72   :       ::localtime_s(&destination, &now);

	lea	rdx, QWORD PTR now$[rbp]
	lea	rcx, QWORD PTR destination$[rbp]
	call	localtime_s

; 73   :       _loggingStream << std::put_time( &destination, "%Y-%m-%d %X" ) << " | ";

	mov	rax, QWORD PTR this$[rbp]
	mov	rax, QWORD PTR [rax+8]
	mov	QWORD PTR tv81[rbp], rax
	lea	r8, OFFSET FLAT:??_C@_0M@EHAPGNAC@?$CFY?9?$CFm?9?$CFd?5?$CFX@
	lea	rdx, QWORD PTR destination$[rbp]
	lea	rcx, QWORD PTR $T6[rbp]
...
; 74   :     
; 75   :     #else
; 76   :       _loggingStream << std::put_time( std::localtime( &now ), "%Y-%m-%d %X" ) << " | ";
; 77   :     #endif
; 78   : 
; 79   :     _loggingStream << message << '\n';
...
; 40   :     : _loggingStream( loggingStream )

	mov	rax, QWORD PTR this$[rbp]
	mov	rcx, QWORD PTR loggingStream$[rbp]
	mov	QWORD PTR [rax+8], rcx

; 42   :     // Let's set the buffering behavior of the stream so the order and context of inserted information is preserved. Force the
; 43   :     // buffer to be flushed after each insertion. One normally doesn't want to do this, but log files are very often viewed in real
; 44   :     // time as the program executes (e.g. tailing a file).  Waiting for the buffer to fill before seeing the contents may give false
; 45   :     // information in such cases.
; 46   :     _loggingStream << std::unitbuf;
...
; 47   : 
; 48   :     // call operator<< so the time stamp and newline get inserted too
; 49   :     *this << "Simple Logger being used and has been successfully initialized";
...
; 573  :             {
	...
	rep stosd
	mov	rcx, QWORD PTR [rsp+264]
	lea	rcx, OFFSET FLAT:__CF01F9AA_time@h
	call	__CheckForDebuggerJustMyCode
...
$LN3:
	...
	rep stosd
	mov	rcx, QWORD PTR [rsp+264]
	lea	rcx, OFFSET FLAT:__61D1BCEE_wchar@h
	call	__CheckForDebuggerJustMyCode
